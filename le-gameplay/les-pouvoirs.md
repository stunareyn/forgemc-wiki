# ⚡ Les Pouvoirs

### 📖 Qu'est-ce qu'un Pouvoir ?

Un **Pouvoir** est une capacité spéciale octroyant des passifs uniques (bonus/malus de statistiques, capacités spéciales). Contrairement aux Atouts, **un seul Pouvoir peut être équipé à la fois**.

### ⚙️ Comment ça fonctionne ?

* 📥 **Découverte** : Utilisez la commande `/pouvoirs` pour consulter la liste complète des pouvoirs disponibles.
* 🔓 **Déblocage** : Chaque pouvoir est débloqué soit gratuitement (pouvoirs basiques), soit à l'achat contre des Gemmes (pouvoirs communs et supérieurs).
* 🎒 **Équipement** : Une fois débloqué, équipez le pouvoir de votre choix depuis le menu `/pouvoirs`. **Un seul pouvoir actif à la fois** , équiper un nouveau pouvoir désactive automatiquement l'ancien.
* ⚖️ **Passifs uniques** : Chaque pouvoir apporte des passifs exclusifs (bonus ET malus), à choisir selon votre style de jeu (minage, combat, exploration...).

{% hint style="warning" %}
Les pouvoirs basiques (Poulet, Poulpe, Lapin) sont offerts lors de votre première connexion au serveur. Votre choix est définitif : vous ne pourrez pas revenir en arrière ni acheter les deux autres pouvoirs basiques dans le futur.
{% endhint %}

### ⚪ Pouvoirs Basiques

#### 🐔 Poulet

**Passifs :**

* Taille du joueur x0.5
* Aucun dégât de chute (≤ 15 blocs)
* -2 cœurs

#### 🐙 Poulpe

**Passifs :**

* Respiration aquatique illimitée
* +25% vitesse de minage sous l'eau

#### 🐰 Lapin

**Passifs :**

* Taille du joueur x0.5
* +50% hauteur de saut
* Aucun dégât de chute (≤ 10 blocs)
* -3 cœurs

***

### 🟢 Pouvoirs Communs

#### 🧑‍🌾 Villageois

**Passifs :**

* +2 cœurs
* 10% de chance d'obtenir x2 cultures lors de la récolte

**Prix : 600 Gemmes**

#### 🐱 Ocelot

**Passifs :**

* +15% de vitesse
* Taille du joueur x0.5
* Aucun dégât de chute (≤ 10 blocs)
* -1 cœur

**Prix : 600 Gemmes**

***

### 🔵 Pouvoirs Rares

#### 🐝 Abeille

**Passifs :**

* +10% de vitesse
* 15% de chance d'obtenir x2 miel lors de l'extraction
* Aucun dégât de chute
* Taille du joueur x0.5
* -3 cœurs

**Prix : 1000 Gemmes**

#### 🕷️ Araignée

**Passifs :**

* Taille du joueur x0.9
* Aucun dégât de chute
* Peut grimper aux murs
* 10% de chance d'obtenir x2 fils/yeux d'araignée en tuant des araignées
* -2 cœurs

**Prix : 1000 Gemmes**

***

### 🟣 Pouvoirs Épiques

#### 🧟 Zombie

**Passifs :**

* +2 cœurs
* Aucune perte de nourriture
* Dégâts de chute dès 2 blocs
* Vision nocturne la nuit
* 10% de chance d'obtenir x2 drops en tuant des zombies

**Prix : 1800 Gemmes**

#### ☠️ Wither Squelette

**Passifs :**

* +3 cœurs
* +1 bloc de portée d'attaque
* Immunité au feu et à la lave
* 10% de chance d'obtenir x2 minerais
* -15% de vitesse

**Prix : 1800 Gemmes**

#### 🔥 Blaze

**Passifs :**

* Immunité au feu et à la lave
* Prend des dégâts au contact de l'eau et de la pluie
* Cuisson automatique des minerais minés
* 10% de chance d'obtenir x2 drops en tuant des blazes

**Prix : 1800 Gemmes**

#### 💥 Creeper

**Passifs :**

* +2 cœurs
* Immunité aux explosions
* -15% de vitesse
* 10% de chance d'obtenir x2 drops en tuant des creepers

**Prix : 1800 Gemmes**

***

### 🟡 Pouvoirs Légendaires

#### 🌌 Enderman

**Passifs :**

* Taille du joueur x1.5
* Prend des dégâts au contact de l'eau et de la pluie
* +50% résistance aux dégâts de projectiles
* Déplacement silencieux
* 10% de chance d'obtenir x2 drops en tuant des endermans

**Prix : 3000 Gemmes**

#### 🏹 Illageois

**Passifs :**

* +15% de dégâts supplémentaires
* +4 cœurs
* -20% de vitesse
* 10% de chance d'obtenir x2 drops en tuant des vindicateurs/évocateurs

**Prix : 3000 Gemmes**

***

### 🔴 Pouvoirs Mythiques

#### 👁️ Warden

**Passifs :**

* +10 cœurs
* Taille du joueur x2
* x2 dégâts de chute
* Effet de résistance permanent
* 10% de chance d'obtenir x2 drops en tuant n'importe quel mob

***

💡 **Astuce :** Chaque pouvoir a ses avantages **et** ses inconvénients, pensez à choisir celui qui correspond le mieux à votre style de jeu (combat, minage, agriculture...) !
